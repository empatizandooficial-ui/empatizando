    ...
    <div className="fixed bottom-6 right-6 z-40 flex flex-col items-end">
      ...
      {!isOpen && (
        <button
          ...
          className="relative group gradient-bg text-white h-16 w-16 rounded-full flex items-center justify-center shadow-glow transition-all hover:scale-105 focus:outline-none"
        >
          <div className="absolute inset-0 rounded-full border-2 border-primary/40 animate-heartbeat"></div>
          <MessageCircleHeart size={30} className="relative z-10" />
        </button>
      )}
    </div>
  );
